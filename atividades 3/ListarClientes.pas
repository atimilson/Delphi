...
procedure TFListarClientes.DBGrid1DblClick(Sender: TObject);
begin
   ID := DBGrid1.DataSource.DataSet.Fields[0].AsString;
   self.Close;
end;

procedure TFListarClientes.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
   ...
   if (Edit1.Text <> '') then
      sql := sql +' and Nome like ' + QuotedStr('%'+Edit1.Text+'%');

   if (Edit2.Text <> '') then
      sql := sql+' and CPF like '+ QuotedStr('%'+Edit2.Text+'%');

   if (Edit3.Text <> '') then
      sql := sql+' and Bairro like ' + QuotedStr('%'+Edit3.Text+'%');
