...
begin
   if ((StrToInt(Edit2.Text)) >= 20 ) then
      ...
      ShowMessage('O valor do Campo é menor que 20');
end;
...
begin
   ...
   if (aux >= 10) then
      ...
      ShowMessage('A soma do valor é ' + IntToStr(aux))
   else
      ShowMessage('O Valor é menor que 10');
end;
...
begin
   ...
   begin
      ...
      if (i = 0) then
         ShowMessage('O valor é divisível por 5')
      else
         ShowMessage('O valor não é divisível por 5');
   end;
end; 
...
begin
  ...
  if (i = 0) And (b = 0) then
     ShowMessage('Divisível por 3 e 5')
  else
     ShowMessage('Não é divisível por 3 e 5');
end;
...
begin
   ...
   begin
      ...
      if (aux >= 7) then
         Memo6.Lines.Add('A situação : Aprovado')
      else if (aux >= 3) and (aux < 7) then
         Memo6.Lines.Add('A situação : Prova Final')
      else
         Memo6.Lines.Add('A situação : Reprovado');
   end
   else
      ShowMessage('Nota 1 ou a Nota 2 está sem com valor');
...
begin
   if(Edit18.Text = 'BRASÍLIA') or (Edit18.Text = 'Brasília')then
...
begin
   ...
   begin
      ...
      //aux:= aux mod StrToInt(Edit20.Text); // Divide o resultado do quociente pelo divisor | se for divisivel o resto será 0

      //aux := (aux * StrToInt(Edit20.Text) = StrToInt(Edit19.Text) )
      if (aux * StrToInt(Edit20.Text) = StrToInt(Edit19.Text)) then
         Memo8.Lines.Add('É divisivel pelo valor')
      else
         Memo8.Lines.Add('Não é divisivel pelo valor');
   end;
///  ok
end;
...
begin
  ...
  if (teste = 0) then
     ...
     ShowMessage('Numero não é inteiro');
...
begin
   if(StrToFloatDef(Edit25.Text,0) > StrToFloatDef(Edit26.Text,0)) then
      Memo11.Lines.Add('O maior numero é '+Edit25.Text)
   else
      Memo11.Lines.Add('O maior numero é  '+Edit26.Text);
   Edit25.Clear;
   Edit26.Clear;
end;
...
//   StringGrid2.ColWidths[0] := -1;//Não exibir uma coluna
...
begin
   ...
   begin
      if(StrToFloatDef(Edit27.Text,0) > StrToFloatDef(Edit28.Text,0)) then
      begin
         Memo12.Lines.Add('O numero menor é '+ Edit28.Text)
      end
      else
         Memo12.Lines.Add('O numero menor é '+ Edit27.Text)
   end
   else
      Memo12.Lines.Add('Os numeros são iguais');
end;
...
begin
   ...
   begin
      ...
      if (var1 > var2) and (var1 > var3) then
         ...
         maior:= var3;
      Memo15.Lines.Add('O maior numero é '+FloatToStr(maior));
   end;
...
begin
   ...
   if (var1 > var2) and (var1 > var3) then
      ...
      maior:= var3;
   memoValor.Lines.Add('O maior numero é '+IntToStr(maior));
...
begin
   ...
   if (var1 < var2+var3) and (var2 < var1+var3) and (var3 < var1 + var2) then
      ...
      Memo20.Lines.Add('Não é um triangulo');
...
begin
   ...
   begin
      ...
      Memo21.Lines.Add('não é um triangulo');
end;
...
begin
   ...
   case aux of
      0: Memo23.Lines.Add('Os nomes são iguais');
...
begin
   ...
   if (nasc > 1900) and (anoAtu > 1900) then
      Memo25.Lines.Add('Sua Idade é ' + IntToStr(anoAtu - nasc))
   else
...
begin
   ...
   if (idade > 25) and (sexo = 0) then
      Memo26.Lines.Add('Nome : '+nome+' Situação : Aceita')
   else
      Memo26.Lines.Add('Nome : '+nome+' Situação : Não Aceita')
...
begin
   ...
   begin
     // aux := sqrt(delta);
      ...
      Memo29.Lines.Add('A equação não posuui Raizes Reais');
...
begin
   ...
   case status of
      ...
      4: Memo30.Lines.Add('viúvo(a)');
         else
            ShowMessage('Não identificado');
   end;
/// -- ok
end;
   ...
   case (selecionado) of
    ...
    begin
       if(StringGrid3.Cells[COLMENSAGEM, StringGrid3.RowCount - 1] <> '')then
          StringGrid3.RowCount := StringGrid3.RowCount + 1;
          StringGrid3.Cells[COLMENSAGEM, StringGrid3.RowCount - 1]:= 'Mes não existe'
...
//   else
//      ShowMessage('Maio é o ultimo Mes');
//   end;
var
   letra: char;
   teste: string;
...
begin
   ...
   begin
      ...
      Memo37.Lines.Add(var1+' não são multiplos de 4')
   end
   else
...
begin
   ...
   if ((angulo >= 0) and (angulo < 90)) or ((angulo >= 180) and (angulo < 270)) then
      Memo40.Lines.Add('Quadrante Impar |'+' O cosseno é : '+ FloatToStr(Cos(angulo)) )
   else if ((angulo >= 90) and (angulo < 180)) or ((angulo >= 270) and (angulo <=360)) then
      Memo40.Lines.Add('Quadrante par |'+' O seno é : '+ FloatToStr(Sin(angulo)))
   else
      ShowMessage('Não é um angulo valido');
end;
